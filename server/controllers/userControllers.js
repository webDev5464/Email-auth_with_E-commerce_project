...
export const UserRegister = async (req, res) => {
  try {
    ...
    otpStorage.set(email, {
      otp,
      expireOTP,
      userData: { username, email, pass: hashPass },
    });
...
export const OtpValidation = async (req, res) => {
  try {
    ...
    if (!storedUserOtpDetail)
      throw new Error("Somethings wrong! Please try again to send OTP");
...
export const UserLogin = async (req, res) => {
  try {
    ...
    const genToken = jwt.sign({ userId: findUser._id }, "secureKey", {
      expiresIn: "30s",
    });
...
export const ResendOtp = async (req, res) => {
  try {
    ...
    res.status(200).send({
      message: "Resend New OTP send chek now",
    });
  } catch (error) {
    res.status(400).send({
      process: false,
